...
### 1.1 什么是 XSS (Cross Site Scripting)

XSS 是一种通过在网页中注入恶意脚本，从而实现攻击目的的漏洞。该漏洞主要发生在对 HTML 不正确拼接的场景中。同时，XSS 也不局限于引用 JavaScript 代码，也可以使用 CSS 等技术来得到窃取用户信息等目的。
...
- **Sources (源)**：攻击者可控制的输入，如 `location.search`, `document.referrer`, `document.cookie`, `location.href` 等
- **Sinks (接收器)**：危险的执行/渲染端点，如 `eval()`, `innerHTML`, `document.write()`, `setTimeout()`, `location.href` 等
...
链接：<https://ctf.aabyss.cn/xss-labs/index.php>

请尝试完成其中直到 Level 7 的题目。
...
### 题目：Author (XCTF-RCTF 2025)

请使用以下命令启动题目：

```bash
docker load -i author.tar
docker run --rm -p 8080:80 -p 3000:3000 author-app.tar
```

端口 8080 是题目环境、3000 是模仿管理员行为的 Bot 服务。

在 Bot 服务处提交 Article ID 后，题目环境会进行一些操作，随后加载给定 URL 的页面。
